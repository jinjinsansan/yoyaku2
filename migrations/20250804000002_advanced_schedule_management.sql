...
ADD COLUMN sync_status TEXT DEFAULT 'none' CHECK (sync_status IN ('none', 'synced', 'sync_failed', 'sync_pending')),
ADD COLUMN buffer_before_minutes INTEGER DEFAULT 0,
ADD COLUMN buffer_after_minutes INTEGER DEFAULT 0,
ADD COLUMN notes TEXT;

-- インデックス作成
CREATE INDEX idx_counselor_calendar_settings_counselor_id ON counselor_calendar_settings(counselor_id);
CREATE INDEX idx_counselor_time_off_counselor_id ON counselor_time_off(counselor_id);
CREATE INDEX idx_counselor_time_off_dates ON counselor_time_off(counselor_id, start_date, end_date);
CREATE INDEX idx_schedule_templates_counselor_id ON schedule_templates(counselor_id);
CREATE INDEX idx_schedule_templates_active ON schedule_templates(counselor_id, is_active);
CREATE INDEX idx_schedule_generation_logs_counselor_id ON schedule_generation_logs(counselor_id);
CREATE INDEX idx_counselor_schedules_template_id ON counselor_schedules(template_id);
CREATE INDEX idx_counselor_schedules_google_event_id ON counselor_schedules(google_event_id);
